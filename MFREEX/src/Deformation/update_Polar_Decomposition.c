...
// static inline int permutation_symbol(i,j,k)
// {
// 	return e[i][j][k];
// }
...
int update_Polar_Decomposition(state_variables * stateNew, state_variables * stateOld, double dt)
{
			...
			m_mlt_3x3(stateOld->V,stateNew->D,stateNew->m_temp1);
			m_mlt_3x3(stateNew->D,stateOld->V,stateNew->m_temp2);
			...
			for ( int i = 0  ; i < 3 ; i++)
			{
				for ( int j = 0 ; j < 3 ; j++)
				{
					for ( int k = 0 ; k < 3 ; k++)
					{
						...
						int j1 = j+1;
						int k1 = k+1;
						double e = ((i1-j1)*(j1-k1)*(k1-i1))/2.00;
			...
			m_sub_3x3(stateNew->m_temp1,stateOld->V, stateNew->m_temp1);
			...
			m_mlt_3x3(stateNew->L,stateOld->V,Vdot);
			m_mlt_3x3(stateOld->V,stateNew->Omega,stateNew->m_temp1);
			m_sub_3x3(Vdot,stateNew->m_temp1,Vdot);


			sm_mlt(dt,Vdot,stateNew->V);
			m_add_3x3(stateNew->V,stateOld->V,stateNew->V);

			// //Find rotation tensor using Hughes, Winget algorithm
			m_ident(stateNew->m_temp1);
			sm_mlt(0.5*dt,stateNew->Omega,stateNew->m_temp2);
			m_sub_3x3(stateNew->m_temp1,stateNew->m_temp2,stateNew->m_temp2);
			m_inverse_small(stateNew->m_temp2,stateNew->m_temp1);

			m_ident(stateOld->m_temp1);
			sm_mlt(0.5*dt,stateNew->Omega,stateNew->m_temp2);
			m_add_3x3(stateOld->m_temp1,stateNew->m_temp2,stateNew->m_temp2);
