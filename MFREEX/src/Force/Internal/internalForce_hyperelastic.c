...
#pragma omp for nowait schedule(dynamic,2) 
	for(int i = 0 ; i < num_int_points ; i++){
		...
		get_dot_defgrad(Fdot,scni[i],velocity);
		B = scni[i]->B;
		...
		double P_a = Le*rho*(C0*Le*div_v*div_v);

		if ( div_v > 0){
			P_a = 0;
		}
		double P_b = C1*div_v*rho*Le*c;
		...
		if ( dim_v == 4)
		{
			...
			double S11_hyd = (Jacobian)*(P_a+P_b)*invF11;
			double S22_hyd = (Jacobian)*(P_a+P_b)*invF22;
