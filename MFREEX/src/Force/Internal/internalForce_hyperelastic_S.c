...
double internalForce_hyperelastic_S(VEC *Fint, MSCNI_OBJ *scni_obj, VEC *disp,
                                    VEC *velocity, VEC *matParams,
                                    char *Material, int is_axi, int dim) {
...
#pragma omp parallel
  {
    ...
    double invF11, invF12, invF13, invF21, invF22, invF23, invF31, invF32,
        invF33;
...
#pragma omp for nowait schedule(dynamic, 1)
    for (int i = 0; i < num_int_points; i++) {
      ...
      if (dim == 2) {
        ...
        Co21 = -F12;
        Co22 = F11;
        invF11 = Co11 / Jacobian;
        invF12 = Co21 / Jacobian;
        invF21 = Co12 / Jacobian;
        invF22 = Co22 / Jacobian;

        // Find velocity gradient

        L11 = Fdot->me[0][0] * invF11 + Fdot->me[0][1] * invF21;
        L12 = Fdot->me[0][0] * invF12 + Fdot->me[0][1] * invF22;
        L21 = Fdot->me[1][0] * invF11 + Fdot->me[1][1] * invF21;
        L22 = Fdot->me[1][0] * invF12 + Fdot->me[1][1] * invF22;
      ...
      double delta_t = (2.00 / sqrt(((lambda + 2 * mu) * MaxB))) *
                       (sqrt(1 + eta * eta) - eta);
      ...
      if (dim_v == 4) {
        ...
        S11 = stressVoigt->ve[0] + S11_hyd;
        S12 = stressVoigt->ve[2];
        S21 = stressVoigt->ve[3];
        S22 = stressVoigt->ve[1] + S22_hyd;

        F11 = scni[i]->F_r->me[0][0];
        F12 = scni[i]->F_r->me[0][1];
        F21 = scni[i]->F_r->me[1][0];
        F22 = scni[i]->F_r->me[1][1];
        ...
        S11 = stressVoigt->ve[0];
        S12 = stressVoigt->ve[2];
        S21 = stressVoigt->ve[3];
        S22 = stressVoigt->ve[1];
      ...
      vm_mltadd(scni[i]->fInt, stressVoigt, scni[i]->B, intFactor,
                scni[i]->fInt);

      for (int k = 0; k < num_neighbours; k++) {
        fIntTemp->ve[2 * scni[i]->sfIndex->ive[k]] += scni[i]->fInt->ve[2 * k];
        fIntTemp->ve[2 * scni[i]->sfIndex->ive[k] + 1] +=
            scni[i]->fInt->ve[2 * k + 1];
      }
    }
