...
MSCNI_OBJ * generate_mscni(voronoi_diagram * voronoi, char * type, int is_stabalised, int is_AXI, meshfreeDomain * Mfree)
{
	...
	MAT * bI_test = m_get(Mfree->num_nodes,dim);
	...
	for ( int i = 0 ; i < num_cells ; i++)
	{
		...
		for ( int k = 0 ; k < num_cell_verticies ; k++){
			...
			printf("length of outside segemnts = %lf \n",l[0]);
			// area of triangle
			...
			for ( int j = 0 ; j < neighbours->max_dim ; j++)
			{
				int indx = neighbours->ive[j];
				// add contribution to bI and cell_sf_index 
				bI_test->me[indx][0] += 0.5*(n[0][0]*l[0])*phi->ve[j];
				bI_test->me[indx][1] += 0.5*(n[0][1]*l[0])*phi->ve[j];
		...
		IVEC * cell_sf_index = iv_get(20);
		int count = 0;
		for ( int k = 0 ; k < Mfree->num_nodes; k++)
		{
			int j = 0;
			int test = 0;
			while ((j < num_cell_verticies) && (test == 0)){
		...
		MAT * bI_n_test = m_get(length_index,dim);
		...
		for ( int k = 0 ; k < cell_sf_index->max_dim; k++)
		{
			...
			for ( int j = 0 ; j < num_cell_verticies ; j++)
			{
				...
				bI_n_test->me[k][0] = bI_test->me[indx][0];
				bI_n_test->me[k][1] = bI_test->me[indx][1];
			}
		...
		MAT * B = generate_Bmat(bI_n_test,dim,is_AXI,-1);
		sm_mlt(1.00/volume,B,B);
