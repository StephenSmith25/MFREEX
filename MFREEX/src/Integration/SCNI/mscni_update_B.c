...
int mscni_update_B(MSCNI_OBJ * mscni_obj,  VEC * disp, voronoi_diagram * voronoi, meshfreeDomain * Mfree, int is_AXI)
{
...
#pragma omp for
	...
	{		
		...
		// update state variable F_r;
		B = mscni_[i]->B;
		neighbours = mscni_[i]->sfIndex;
		F_r = mscni_[i]->F_r;
		get_defgrad(temp_F, B, neighbours,F_r, disp);
		m_copy(temp_F,mscni_[i]->F_r);


		// free previously allocated memory
		for ( int k = 0 ; k < mscni_[i]->num_sub_cells ; k++)
		{
			M_FREE(mscni_[i]->stabalised_B[k]);
		}
		free(mscni_[i]->stabalised_volumes);
		free(mscni_[i]->stabalised_B);
		M_FREE(mscni_[i]->B);
		IV_FREE(mscni_[i]->sfIndex);
		V_FREE(mscni_[i]->fInt);
