...
#if DIM == 2
  double x_m_xI[2] = {x[0] - x_I[0], x[1] - x_I[1]};
  norm_distance = ((x_m_xI[0]) * (invMI->me[0][0] * x_m_xI[0] +
                                  invMI->me[0][1] * x_m_xI[1]) // row 1
                   + (x_m_xI[1]) * (invMI->me[1][0] * x_m_xI[0] +
                                    invMI->me[1][1] * x_m_xI[1]) // row 2
  );

#elif DIM == 3
  double x_m_xI[3] = {x[0] - x_I[0], x[1] - x_I[1], x[2] - x_I[2]};
  norm_distance = ((x_m_xI[0]) * (invMI->me[0][0] * x_m_xI[0] +
                                  invMI->me[0][1] * x_m_xI[1] +
                                  invMI->me[0][2] * x_m_xI[2]) // row 1
                       + (x_m_xI[1]) * (invMI->me[1][0] * x_m_xI[0] +
                                        invMI->me[1][1] * x_m_xI[1] +
                                        invMI->me[1][2] * x_m_xI[2]); // row 2
                   + (x_m_xI[1]) * (invMI->me[2][0] * x_m_xI[0] +
                                    invMI->me[2][1] * x_m_xI[1] +
                                    invMI->me[2][2] * x_m_xI[2]) // row 3
  );
#endif
...
static inline MATERIAL_POINT *create_material_point(double coords[3],
                                                    double volume) {
...
MATERIAL_POINTS *create_material_points(void *cells, int IS_AXI_I, int dim,
                                        ...
                                        CELLS *grid, double rho, double beta,
...

{
  ...
  if (strcmp(integration_type, "SCNI") == 0) {
    ...
    for (int i = 0; i < material_points->num_material_points; i++) {
      ...
      MPS[i]->stateOld =
          new_material_state(MPS[i]->temperature, material_type, dim, IS_AXI);
      MPS[i]->stateNew =
          new_material_state(MPS[i]->temperature, material_type, dim, IS_AXI);
...

  {
    ...
    QUAD_TRIANGLE *quad_triangles = create_triangle_quadrature_points(
        tri_points, triangles, number_of_triangles, quad_orders);
    ...
    MATERIAL_POINT **MPS =
        malloc(number_of_quadrature_points * sizeof(MATERIAL_POINT));
    ...
    for (int i = 0; i < number_of_quadrature_points; i++) {
      ...
      // Create the material points
      MPS[i] =
          create_material_point(point_coords, quad_triangles->VOLUMES->ve[i]);
      ...
      MPS[i]->shape_function =
          mls_shapefunction_materialpoint(MPS[i], 2, mfree->nodes);

      // getting Bmat
      // Strain Displacement relationship
      MPS[i]->B = BMAT(MNULL, MPS[i]->shape_function, dim, IS_AXI,
                       MPS[i]->coords_n_1[0]);
      ...
      if (mfree->temperatures != NULL) {
        for (int k = 0; k < MPS[i]->num_neighbours; k++) {
          ...
          MPS[i]->temperature +=
              MPS[i]->shape_function->phi->ve[k] * mfree->temperatures[index];
        }
      }
...
static inline int delete_material_point() { return 0; }
// static call_count = 0;
...
int write_material_points(char *filename, MATERIAL_POINTS *MPS) {
  ...
  for (int i = 0; i < MPS->num_material_points; i++) {
    ...
    fprintf(fp, "%lf,%lf,%lf,%lf,%lf,%lf\n", MPS->MP[i]->coords_n_1[0],
            MPS->MP[i]->coords_n_1[1], MPS->MP[i]->coords_n_1[2], s11, s22,
            s12);
  }
...
MATERIAL_POINT *update_material_point(MATERIAL_POINT *MP, CELLS *grid,
                                      MAT *NODES, VEC *nodal_mass) {
  ...
  for (int k = 0; k < MP->num_neighbours; k++) {
    ...
    nodal_mass->ve[index] += MP->INTEGRATION_FACTOR * MP->volume * MP->rho *
                             MP->shape_function->phi->ve[k];
  }
...
int RangeSearchMaterialPoint(MATERIAL_POINT *MP, MAT *nodes, CELLS *cells) {
