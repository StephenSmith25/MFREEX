...
static double epsilon_penalty = -10000;
...
void internal_force_buckley(void *threadarg){
	...
	for ( int k = 0 ; k < num_neighbours; k++){
		...
		// assemble forces 
		internal_force_struct->RPEN->ve[2*index] += epsilon_penalty*MP->shape_function->phi->ve[k]*
			e_x*intFactor;
		internal_force_struct->RPEN->ve[2*index+1] += epsilon_penalty*MP->shape_function->phi->ve[k]*
			e_y*intFactor;
