...
Din_b = (13.97+14.43)/2+1;
Dout_b = (23.19+22.50)/2-1;
...
for i = 1:length(theta)
    nodes1(count,:) = [Rin_bot*cosd(theta(i)), -vDim + Rin_bot*sind(theta(i))];
    count = count +1;
...
count = count + N2;
nodes1(count:1:count+N3-1,:) = [linspace(Din_t/2,Din_t/2,N3)',linspace(L_m+L_t,L_t+L_m+L_t1,N3)'];
count = count + N3;

height = nodes1(end,2);
%% Essential boundary nodes
nodes1(count:1:count+N4-1,:) = [linspace(Din_t/2,Dout_t/2,N4)',linspace(height,height,N4)'];
count = count + N4;
%% Outside nodes
% right wall
...
% Bottom edge
nodes1(count:1:count+N5-1,:) = [linspace(0,0,N5)',linspace(-Rout_bot,-Rout_bot+thickness_bot,N5)'];
