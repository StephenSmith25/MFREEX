...
int main(int argc, char** argv) {
	...
	// READ MESH FILE GENERATED BY 
 	DOMAIN * domain = read_intput_mesh(argv[1]);
	...
	BOUNDING_BOX * bounding_box = create_bounding_box(MIN_X, MAX_X,
	MIN_Y, MAX_Y, 0, 0);
	...
	VEC * dI = v_get(xI->m);
	// meshfree domain
	meshfreeDomain mfree = {.nodes = xI, .di = dI, .num_nodes = xI->m, .dim = DIM, .IS_AXI = ISAXI,
	...
	// Loop over each blockset -
	// make material points 
	/* 	PROTOTYPE  
		- Takes domain
		- loops over each blockset
		- Find material points based on either triangle, quad, or tet integration
		- Gets shape function at material points
		- Sets solution matricies 
	*/
