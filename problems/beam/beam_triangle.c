...
int main(void) {
  ...
  shape_function_container *sf_nodes =
      mls_shapefunction(mfree.nodes, 2, &mfree);
  ...
  // create saerch celsl
  BOUNDING_BOX *bounding_box = create_bounding_box(0, 21, 0, 2, 0, 0);
  ...
  if (strcmp(integration_type, "SCNI") == 0) {
    ...
    for (int i = 0; i < number_of_material_points; i++) {
      ...
      material_points->MP[i]->stateNew =
          new_material_state(0, BUCKLEY, dim, is_AXI);
  ...
  for (int i = 0; i < num_nodes_trac; i++) {
    traction_nodes_coords->me[i][0] =
        mfree.nodes->me[traction_nodes->ive[i]][0];
    traction_nodes_coords->me[i][1] =
        mfree.nodes->me[traction_nodes->ive[i]][1];
  }
  // get shape function and traction nodes
  shape_function_container *phi_traction =
  ...
  while (t_n < t_max) {
    ...
    for (int i = 0; i < neighbours->max_dim; i++) {
      // x
      Fext_n_1->ve[2 * neighbours->ive[i]] =
          phi->ve[i] * tipLoad * dir_load->ve[0];
      // y
      Fext_n_1->ve[2 * neighbours->ive[i] + 1] =
          phi->ve[i] * tipLoad * dir_load->ve[1];
    }
  ...
  delta = ((end3.tv_sec - start3.tv_sec) * 1000000u + end3.tv_usec -
           start3.tv_usec) /
          1.e6;
