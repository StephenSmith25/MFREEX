...
int main(int argc, char **argv) {
  ...
  // matParams->ve[2] = (0.00/4.00)*(1.8165e6) + (4.000/4.00)*(1.7057e6); //
  // mu*_0 matParams->ve[3] = (0.00/4.00)*(342.61)+ (4.00/4.00)*(328.76); //
  // Tinf matParams->ve[6] = (0.00/4.00)*(56.09)+ (4.00/4.00)*(67.47); // Cv
  ...
  double *all_temperatures =
      malloc((number_of_stress_points + numnodes) * sizeof(double));
  ...
  for (int i = 0; i < number_of_triangles; i++) {
    ...
    stress_points->me[i][0] =
        (xI->me[index_1][0] + xI->me[index_2][0] + xI->me[index_3][0]) / 3.00;
    stress_points->me[i][1] =
        (xI->me[index_1][1] + xI->me[index_2][1] + xI->me[index_3][1]) / 3.00;

    int indx = numnodes + i;
    all_temperatures[indx] =
        (all_temperatures[index_1] + all_temperatures[index_2] +
         all_temperatures[index_3]) /
        3.00;
  }
  ...
  vor = generate_voronoi(ALL_POINTS->base, boundaryNodes, ALL_POINTS->m,
                         numBoundary, 2);
  ...
  meshfreeDomain mfree = {.nodes = xI,
                          ...
                          .is_constant_support_size = is_constant_support_size,
                          .dmax_radial = dmax,
                          .dmax_tensor = dmax_tensor,
                          .beta = beta};
  ...
  material_points->MP =
      malloc(number_of_material_points * sizeof(MATERIAL_POINT *));
  ...
  for (int i = 0; i < number_of_material_points; i++) {
    ...
    if (i < numnodes) {
      ...
      material_points->MP[i]->num_neighbours =
          _scni_obj->scni[i]->sfIndex->max_dim;
      material_points->MP[i]->shape_function =
          malloc(1 * sizeof(shape_function));
      material_points->MP[i]->shape_function->phi = _scni_obj->scni[i]->phi;

      material_points->MP[i]->INTEGRATION_FACTOR =
          2 * PI * _scni_obj->scni[i]->r;
      material_points->MP[i]->volume = _scni_obj->scni[i]->area;

      material_points->MP[i]->stateNew =
          new_material_state(all_temperatures[i], BUCKLEY, dim, is_AXI);
      material_points->MP[i]->stateOld =
          new_material_state(all_temperatures[i], BUCKLEY, dim, is_AXI);
  ...
  for (int i = 0; i < number_of_material_points; i++) {
    ...
    volumeaa += material_points->MP[i]->volume *
                material_points->MP[i]->INTEGRATION_FACTOR;
  }
  ...
  // Traction
  IVEC *traction_nodes;
  getBoundary(&traction_nodes, boundaryNodes, numBoundary, nodalMarkers,
              numnodes, 2);
  ...
  eb1->dofFixed = 3;
  getBoundary(&eb1->nodes, boundaryNodes, numBoundary, nodalMarkers, numnodes,
              5);
  ...
  eb2->dofFixed = 1;
  getBoundary(&eb2->nodes, boundaryNodes, numBoundary, nodalMarkers, numnodes,
              4);
  // iv_addNode(eb2->nodes,traction_nodes->ive[traction_nodes->max_dim -
  // 1],'e');
  ...
  shape_function_container *phi_contact =
      mls_shapefunction(contact_nodes_coords, 1, &mfree);
  m_foutput(stdout, contact_nodes_coords);

  IVEC *eb4_nodes;
  getBoundary(&eb4_nodes, boundaryNodes, numBoundary, nodalMarkers, numnodes,
              6);
  ...
  for (int i = 0; i < num_nodes_eb4; i++) {
    contact_mould_nodes_coords->me[i][0] =
        mfree.nodes->me[eb4_nodes->ive[i]][0];
    contact_mould_nodes_coords->me[i][1] =
        mfree.nodes->me[eb4_nodes->ive[i]][1];
  }
  // get shape function and contact nodes
  shape_function_container *phi_contact_mould =
      mls_shapefunction(contact_mould_nodes_coords, 1, &mfree);
  ...
  shape_function_container *sf_nodes =
      mls_shapefunction(mfree.nodes, 1, &mfree);
  ...
  double chokedMassRate =
      flowRate(0.528, tLine, pLine * 1e6, rLine, aReduced, gammaLine);
  ...
  state_variables **state_n =
      new_material_states(temperatures, mfree.num_nodes, BUCKLEY_MATERIAL,
                          PLASTIC_MATERIAL, dim, is_AXI);
  ...
  // while ( n < 2)
  {
    ...
    //  double v_rod_poly = 7*a0*pow(x,6) + 6*a1*pow(x,5) + 5*a2*pow(x,4) +
    //  4*a3*pow(x,3) + 3*a4*pow(x,2)
    //  + 2*a5*pow(x,1) +a6;
    ...
    if (disp_rod_n < DISP_ROD_MAX) {
      ...
      double x = t_n_1;
      // disp_rod_n_1 = a0*pow(x,7) + a1*pow(x,6) + a2*pow(x,5) + a3*pow(x,4) +
      // a4*pow(x,3) + a5*pow(x,2) +a6*pow(x,1) + a7; disp_rod_n_1 =
      // disp_rod_n_1;
    ...
    if (t_n_1 > PRESASURE_DELAY) {
      ...
      if (pRatio <= 0.528) {
        ...
        massAir +=
            flowRate(pRatio, tLine, pLine_n * 1e6, rLine, aReduced, gammaLine) *
            delta_t;
      }
      pre_n_1 =
          ((P0 * (volume - volumeInitial) + 1000 * massAir * rLine * tLine) /
           (volume + vDead));
    } else {
      pre_n_1 = 1e-9;
    }
...
#pragma omp parallel for num_threads(NUMBER_OF_THREADS)
    ...
    for (int i = 0; i < mfree.num_nodes; i++) {
      ...
      Wkin += 0.5 *
              (v_n_1->ve[2 * i + 1] * nodal_mass->ve[i] * v_n_1->ve[2 * i + 1]);
    }
    ...
    volume_t = volume;
    // Store previous time step quanities for the kinematic, and force
    // variables.
    ...
    if (n % WRITE_FREQ == 0)
      printf("%i  \t  %lf %10.2E %lf %lf %lf %10.2E \n", n, t_n, Wbal, pre_n_1,
             disp_rod_n_1, volume / 1e3, delta_t);
  }
  ...
  // long elapsedExp = (endExp.tv_sec-endPre.tv_sec) +
  // (endExp.tv_usec-endPre.tv_usec)/1000000.0;

  // printf("Post processing took %ld seconds \n Explicit routine took %ld
