...
int main(int argc, char **argv) {
  ...
  // CREATE NODES
  //
  BOUNDING_BOX *bounding_box = create_bounding_box(-1, 40, -150, 100, 0, 0);
  ...
  for (int i = 0; i < cells->ny; i++) {
    ...
    for (int j = 0; j < cells->nx; j++) {
      ...
      fprintf(fp, "%lf,%lf,%lf,%lf\n", cells->cells[i][j].x[0],
              cells->cells[i][j].x[1], cells->cells[i][j].y[0],
              cells->cells[i][j].y[1]);
    }
  }
  ...
  meshfreeDomain mfree = {.nodes = xI,
                          ...
                          .is_constant_support_size = constant_support_size,
                          .dmax_radial = dmax,
  ...
  /* ---------------------------------------------------------------------------*/
  /* ---------------CREATE THE MATERIAL ( INTEGRATION)
   * POINTS-------------------*/
  /* ---------------------------------------------------------------------------*/
  ...
  if (strcmp(integration_type, "SCNI") == 0) {
    ...
    material_points =
        create_material_points(vor, is_AXI, dim, integration_type,
                               material_type, cells, rho, beta, &mfree);
    ...
    material_points =
        create_material_points(tri, is_AXI, dim, integration_type,
                               material_type, cells, rho, beta, &mfree);
  ...
  // 1],'e'); iv_addNode(eb2->nodes,traction_nodes->ive[0],'e');
  ...
  // while ( n < 10000)
  {
    ...
    double v_rod_poly = 7 * a0 * pow(x, 6) + 6 * a1 * pow(x, 5) +
                        5 * a2 * pow(x, 4) + 4 * a3 * pow(x, 3) +
                        3 * a4 * pow(x, 2) + 2 * a5 * pow(x, 1) + a6;
    ...
    if (disp_rod_n < DISP_ROD_MAX) {
      ...
      disp_rod_n_1 = a0 * pow(x, 7) + a1 * pow(x, 6) + a2 * pow(x, 5) +
                     a3 * pow(x, 4) + a4 * pow(x, 3) + a5 * pow(x, 2) +
                     a6 * pow(x, 1) + a7;
    ...
    for (int i = 0; i < eb3_nodes->max_dim; i++) {
      ...
      if (distanceProj > 0) {
        ...
        f1Cor = 1 *
                (2 * distanceProj * msNormal->me[0][0] *
                 nodal_mass->ve[eb3_nodes->ive[i]]) /
                pow(delta_t, 2);
        f2Cor = 1 *
                (2 * distanceProj * msNormal->me[0][1] *
                 nodal_mass->ve[eb3_nodes->ive[i]]) /
                pow(delta_t, 2);
    ...
    if ((n % WRITE_FREQ == 0) || (n == 1)) {
      ...
      snprintf(filename, 50, "MaterialPoints/materialpoints_%d%s", fileCounter,
               ".txt");
      write_material_points(filename, material_points);

      snprintf(filename, 50, "MaterialPoints/Domains/domains_%d%s", fileCounter,
               ".txt");
...
// update_material points
      ...
      if (n % UPDATE_FREQUENCEY == 0) {
        material_points->MP[i] = update_material_point(
            material_points->MP[i], cells, XI_n_1, NODAL_MASS[ID]);
      }
#endif
    }
    ...
    if (n % PRINT_FREQ == 0) {
      if (print_count % 40 == 0) {
        printf("Iteration |    Time(s)  |   Energy    |  Pressure(MPa) | "
               "Volume (mL) | EXT  | MASS | INT |\n");
      }
      printf("%7i   |  %10.2E | %10.2E  |    %6lf    |   %7lf  |   %7lf  |  "
             "%7lf  |  %7lf    | \n",
             n, t_n, Wbal, pre_n_1, volume / 1e3, v_sum(Fext_n_1),
             v_sum(nodal_mass), v_sum(Fint_n_1));
      ++print_count;
    }
  }
