...
int main(int argc, char** argv) {
	...
	eb4->dofFixed = 1;
	setUpBC(eb4,inv_nodal_mass,&mfree);
	m_foutput(stdout,eb4->coords);
	///////////////////////////////////////////////////////////////
	...
	// 	}
		...
		// for ( int i =0 ; i < eb4->nodes->max_dim ; i++)
		// {
		// 	int index = eb4->nodes->ive[i];
		// 	d_n_1->ve[2*index] = 0;
		// 	v_n_h->ve[2*index] = 0;
		// }
		...
		{
			int index = eb3->nodes->ive[i];
			d_n_1->ve[2*index+1] = 0;
			v_n_h->ve[2*index+1] = 0;
		}

		double ubar = -(PUNCH_SPEED*t_n_1);

		for ( int i =0 ; i < eb2->nodes->max_dim ; i++)
		{
			int index = eb2->nodes->ive[i];
			d_n_1->ve[2*index+1] = ubar;
			v_n_h->ve[2*index+1] = -PUNCH_SPEED;
		}
